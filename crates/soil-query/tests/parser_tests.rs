...
use soil_query::{Result, SoilProfile};

#[test]
fn test_parse_single_profile_gi() -> Result<()> {
    // Arrange: Read the GI.SOL file
    let content =
        std::fs::read_to_string("tests/data/GI.SOL").expect("Failed to read GI.SOL test file");
...
fn test_parse_multiple_profiles_bm() -> Result<()> {
    ...
    let content =
        std::fs::read_to_string("tests/data/BM.SOL").expect("Failed to read BM.SOL test file");
...
fn test_missing_values_represented_as_none() -> Result<()> {
    ...
    assert_eq!(
        first_layer.slcf, None,
        "SLCF should be None for missing data"
    );
    assert_eq!(
        first_layer.slhb, None,
        "SLHB should be None for missing data"
    );
    assert_eq!(
        first_layer.sadc, None,
        "SADC should be None for missing data"
    );
